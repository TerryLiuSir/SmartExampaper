...
public class Page {
    ...
    @Generated(hash = 1513140207)
    private transient PageDao myDao;
    @Generated(hash = 1693897029)
    public Page(Long id, int pageNumber, @NotNull Long bookId, String chapter) {
        this.id = id;
    ...
    public List<Row> getRows() {
        if (rows == null) {
            ...
            RowDao targetDao = daoSession.getRowDao();
            List<Row> rowsNew = targetDao._queryPage_Rows(id);
            synchronized (this) {
                if (rows == null) {
                    rows = rowsNew;
                }
