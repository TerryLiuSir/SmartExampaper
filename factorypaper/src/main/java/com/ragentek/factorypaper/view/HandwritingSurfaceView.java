package com.ragentek.factorypaper.view;
...
import com.ragentek.factorypaper.R;
...
public class HandwritingSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public synchronized void drawPoint(final Point dos, final boolean clear, boolean isMoveTo) {
        ...
        if (isMoveTo) {
            mPath.moveTo(dos.x, dos.y);
        } else {
            mPath.lineTo(dos.x, dos.y);
        }

    }

    public synchronized void loadAnwserArea(final ArrayList<Point> dos, final boolean clear) {
